...
4. Задайте имя коллекции (например, «Квесты»).

<div align="center">
  <img src="../images/quests.png" alt="квесты" >
</div>
...
## Упоминания и быстрые ссылки

Для ускорения коммуникации и навигации в проекте реализована система упоминаний и ссылок с помощью специальных символов.

### Упоминание участников
Чтобы обратиться к конкретному человеку в обсуждении, комментарии, описании элемента или задачи, используйте символ `@` и начните вводить имя.  
Пример: `@ivanov` – выбранный пользователь получит уведомление.

### Ссылки на элементы и задачи
Для быстрого перехода к любому элементу, папке или задаче применяйте символ `#` и идентификатор (отображается в адресной строке или карточке объекта).  
Примеры:
- `#task-42` – создаст кликабельную ссылку на задачу с идентификатором `task-42`
- `#char/main-hero` – ссылка на элемент «Главный герой» в папке `char`

Система автоматически подставит название объекта и оформит ссылку. При вставке такого кода в текст все участники смогут мгновенно перейти к нужному элементу или задаче.

### Как это работает
- В полях ввода с поддержкой форматирования (обсуждения, комментарии, описания) достаточно ввести `@` или `#` – появится выпадающий список для выбора.
- При упоминании пользователя или вставке ссылки на элемент/задачу заинтересованные лица автоматически получают уведомление (если включены соответствующие настройки).

Таким образом, вы можете собирать в одном сообщении контекст из разных уголков проекта, связывая людей, элементы и задачи без ручного копирования длинных путей.
